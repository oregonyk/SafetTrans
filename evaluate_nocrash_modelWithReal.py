...
def main(args):
    ...
    writtenPath_model = find_available_filename(
        "ScenarioNewAddedConfig/modelWithReal_model")  # TODO need to connect to the real dataset
...
class calraProblem(ElementwiseProblem):
    def __init__(self, args, runner, binaryGood, TITGood, TETGood, collisionGood, nocollisionGood):
        ...
        self.TETRetrainModelPath = None
        self.TITRetrainModelPath = None
        self.binaryRetrainModelPath = None
        self.collisionRetrainModlePath = None
        self.nocollisionRetrainModlePath = None

        # all model are good, then no need to retrain the model else training
        if (binaryGood and TETGood and TITGood and collisionGood and nocollisionGood):
            self.modelTrainingSign = False
        else:
            self.modelTrainingSign = True
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        th1 = 0.1
        SD_acc = 0.8
        TIT_RMSE_threshold = 0.4
        TET_RMSE_threshold = 0.4
        SaftyDegree_collision_RMSE_threshold = 1.3
        SaftyDegree_nocollision_RMSE_threshold = 1.3  # TODO
        # SD_acc = 0.7
        # TIT_RMSE_threshold = 0.8
        # TET_RMSE_threshold = 0.8
        # SaftyDegree_collision_RMSE_threshold = 2.3
        # SaftyDegree_nocollision_RMSE_threshold = 2.3  # TODO
        ...
        if (self.modelTrainingSign == False):
            ...
            print(output)
            # print(f"SD:{output[0]:.3f} {output[4]:.3f}\tTET:{output[2]:.3f}\tTIT:{output[1]:.3f}\tCHANGED{changed:.2f}\tMAX_PER：{maxPercent:.4f}\t",x.tolist(), )
            # print(
            #     f"SD:{output[0]:.3f}\tTET:{output[2]:.3f}\tTIT:{output[1]:.3f}\tCHANGED{changed:.2f}\tMAX_PER：{maxPercent:.4f}\t",
            #     x.tolist(), )
        else:
            global writtenPath_real
            # TODO need to connect to the real dataset
            f_r_real = open(writtenPath_real, "a+")
            writer_real = csv.writer(f_r_real)
            ...
            print(f"Collision|TotalRunningTime：{hadCollision}|{xWithResultCounter}")
            # global writtenPath_real
            # TODO need to vertify the singal training number of the running
            print((((xWithResultCounter + 1) % retrainThreshold) == 0), hadCollision > 9)
            print(
                f"SD {self.binaryGood}, Co{self.collisionGood}, No{self.nocollisionGood}, TET {self.TETGood}, TIT {self.TITGood}")
            if ((((
                          xWithResultCounter + 1) % retrainThreshold) == 0) and hadCollision > 9):  # batch size = 200; in SMOTE, need to use neighborhod >5
                if (self.binaryGood == False):
                    accuracy, newModelPathA = self.reTrainBinaryModel(binaryModelPath, MinMaxScalerSDPath,
                                                                      writtenPath_real)
                    if (accuracy > SD_acc):
                        self.binaryGood = True
                        self.binaryRetrainModelPath = newModelPathA
                if (self.collisionGood == False):
                    RMSE, newModelPathD = self.reTrainCollision(CollisionModelPath, MinMaxScalerCollisionSDPath,
                                                                writtenPath_real)
                    if (RMSE < SaftyDegree_collision_RMSE_threshold):
                        self.collisionGood = True
                        self.collisionRetrainModlePath = newModelPathD
                if (self.nocollisionGood == False):
                    RMSE, newModelPathE = self.reTrainNocollision(NocollisionModelPath, MinMaxScalerNocollisionSDPath,
                                                                  writtenPath_real)
                    if (RMSE < SaftyDegree_nocollision_RMSE_threshold):
                        self.nocollisionGood = True
                        self.nocollisionRetrainModlePath = newModelPathE
                if (self.TETGood == False):
                    RMSE, newModelPathB = self.reTrainTET(TETmodelPath, MinMaxScalerPath, writtenPath_real)
                    if (RMSE < TET_RMSE_threshold):
                        self.TETGood = True
                        self.TETRetrainModelPath = newModelPathB
                if (self.TITGood == False):
                    RMSE, newModelPathC = self.reTrainTIT(TITmodelPath, MinMaxScalerPath, writtenPath_real)
                    if (RMSE < TIT_RMSE_threshold):
                        self.TITGood = True
                        self.TITRetrainModelPath = newModelPathC
            if (
                    self.TITGood and self.TETGood and self.binaryGood and self.nocollisionGood and self.collisionGood):
                self.modelTrainingSign = False
    ...
    def Prediction(self, x, MinMaxScalerSDPath, MinMaxScalerPath, MinMaxScalerCollisionSDPath,
        ...
        return [output4, output2, output3, output1]
