...
class NoCrashEvalScenario(RouteScenario):
    ...
    def _set_route(self, hop_resolution=1.0):
        ...
        start = Transform(Location(x=237.010, y=195.3063, z=0.0333), Rotation(pitch=0.005, yaw=-179.2628, roll=0.000))
    ...
    def _initialize_actors(self, config):
        ...
        npc_car_x = 230.010
        npc_car_y = 195.3063
        spawn_point_car = [Transform(Location(x=npc_car_x, y=npc_car_y, z=0.0333),
                                    Rotation(pitch=0.005, yaw=-179.2628, roll=0.000))]
        new_actors = CarlaDataProvider.request_new_batch_actors('vehicle.tesla.model3',
                                                                car_amount,
                                                                spawn_point_car,
                                                                autopilot=False,
        ...
        batch = []
        # add NPC vehicle
        if self.town_name == 'Town01' and ped_amount == 0:
            ...
            vehicle_bp = CarlaDataProvider._blueprint_library.filter('vehicle.tesla.model3')
            if walker_bp.has_attribute('is_invincible'):
                walker_bp.set_attribute('is_invincible', 'false')
            # spawn_point_one = Transform(Location(x=200.670166, y=195.148956, z=0.30000), Rotation(pitch=0, yaw=179.999756, roll=0))
            #NOTE walker position need to be change along with scenario_manager.py pos

            walker_x = 210.670166
            walker_y = 190.148956
            spawn_point_one = Transform(Location(x=walker_x, y=walker_y, z=0.30000))
            CarlaDataProvider.get_world().debug.draw_string(Location(x=walker_x, y=walker_y, z=0.30000), 'x', draw_shadow=False,
                                                            color=carla.Color(r=0, g=255, b=0), life_time=100000,
                                                            persistent_lines=True)


            #vehicle_one = CarlaDataProvider.get_world().spawn_actor(vehicle_bp, spawn_point_car)

            #batch.append(carla.command.SpawnActor(vehicle_bp, spawn_point_car))
