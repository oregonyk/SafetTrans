...
class ScenarioManager(object):
    ...
    def _tick_scenario(self, timestamp):
        ...
        if self._timestamp_last_run < timestamp.elapsed_seconds and self._running:
            ...
            # top view : every step   every 10 step
            if self._pan%20 == 0:
            ...
            if self.npc_id is not None and self._pan>200:
                npc_car = CarlaDataProvider._carla_actor_pool[self.npc_id - 1]
                npc_car.apply_control(carla.VehicleControl(brake=1.0,hand_brake=True))
            else:
                npc_car = CarlaDataProvider._carla_actor_pool[self.npc_id - 1]
                npc_car.apply_control(carla.VehicleControl(throttle = 0.5))
            if self.npc_id is not None and self._pan>15000:
                npc = CarlaDataProvider._carla_actor_pool[self.npc_id]
                ...
                if npc is not None:
                    ...
                    control.speed = 100
                    # print(npc.get_location())
                    if npc.get_location().y > 193:  # 我是提前知道了大概转头的y位置

                        self.revert_flag = True  # 到了就转头
                    if npc.get_location().y < 193:
                        self.revert_flag = False
                    if self.revert_flag:
    ...
    def get_nocrash_analyze_data(self):
        return self.TET, self.TIT, self.average_dacc ,self.TLCJ , self.ALCJ, self.JerkAVG
